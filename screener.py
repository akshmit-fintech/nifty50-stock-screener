...
def _apply_filters(
    ...
    All conditions are AND-ed together. Missing/None conditions are skipped.
    """
...
def run_preset_scan(
    ...
    """
    Run a prebuilt scan preset on already-calculated indicators.

    Args:
        preset_name: Key from SCAN_PRESETS dict
        indicators_df: DataFrame with indicator columns from calculate_all_indicators
    ...
    if indicators_df.empty:
        return pd.DataFrame(), failed_tickers, 0

    filters = SCAN_PRESETS[preset_name]["filters"]
    result = _apply_filters(indicators_df, **filters)

    return result, list(set(failed_tickers)), len(indicators_df)
...
def run_custom_screener(
    ...
    """
    Run a custom screener on already-calculated indicators.
...
def load_and_calculate_all(
    progress_callback: Optional[Callable[[int, int, str], None]] = None
) -> tuple[pd.DataFrame, list[str]]:
    """
    Fetch ALL Nifty 50 stock data and calculate indicators.
    This should be called ONCE per session and cached.

    Args:
        progress_callback: Optional callback(current, total, ticker) for UI progress

    Returns:
        Tuple of (indicators DataFrame, failed tickers list)
    """
    ...
    if not indicators_list:
        return pd.DataFrame(), list(set(failed))

    df = pd.DataFrame(indicators_list)
    return df, list(set(failed))
